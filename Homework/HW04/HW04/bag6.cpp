...
#include <cassert>
#include <cstdlib>
#include "bag6.h" // Include the implementation.
...
// Header file used: bintree.h
{
	...
	//Add first node of the binary search tree
	if (root_ptr == NULL)
	{
		root_ptr = new binary_tree_node<Item>(entry);
		return;
	}
	//move down tree and add new leaf
	else {
		cursor = root_ptr;
		binary_tree_node<Item>* new_entry;
		new_entry = new binary_tree_node<Item>(entry);
		bool finished = false;

		while (!finished) {
			if (cursor->data() < entry) {
				if (cursor->right() == NULL) {
					cursor->set_right(new_entry);
					finished = true;
				}
				cursor = cursor->right();
			}
